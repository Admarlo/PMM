...
public class DataBaseHelper {
    ...
    private static final String DATABASE_CREATE_TODOLIST =
            "create table " + DATABASE_TABLE_TODOLIST + " (" +
                    SL_ID + " integer primary key, " + SL_ITEM + " text not null, " +
    ...
    public Cursor getItems() {
        return mDb.query(DATABASE_TABLE_TODOLIST, new String[]{SL_ID, SL_ITEM, SL_PLACE, SL_IMPORTANCE},
    ...
    public Cursor getItem(int itemId) {
        return mDb.rawQuery(" select " + SL_ITEM + "," + SL_PLACE + "," + SL_DESCRIPTION + "," + SL_IMPORTANCE + "," + SL_ID + " from " + DATABASE_TABLE_TODOLIST + " where " + SL_ID + "= ?", new String[]{Integer.toString(itemId)});
    }
